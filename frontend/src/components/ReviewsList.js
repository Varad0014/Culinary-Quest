...
=======
import { Tilt } from "react-tilt";
>>>>>>> 04ade6a (All changes regrading docker)
...
function ReviewsList({ user }) {
  ...
  return (
    ...
    <div>
      <Link to={`/restaurants/${restaurantId}/reviews/new`} className="btn btn-outline-primary col-lg-3 mx-1 my-5">
        Add Review
      </Link>
      <div className="row">
        {reviews.map((review) => {
...
=======
        <Link to={`/restaurants/${restaurantId}/reviews/new`}>Add Review</Link>
>>>>>>> 04ade6a (All changes regrading docker)
      </div>
    </div>
  );
